...
fn wall_cell_budget(game: &Game) -> usize {
...
fn shrink_walls_to_budget(game: &mut Game, now: i64) -> bool {
...
pub fn sim_tick(game: &mut Game) {
    ...
    let walls_changed = maybe_spawn_wall(game, now)
        | shrink_walls_to_budget(game, now)
        | sweep_walls(game, now)
        | sweep_portal_fx(game, now);
